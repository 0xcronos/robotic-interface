...
conveyor_belt = ConveyorBelt("/dev/cu.usbserial-140", 9600)
...
def detect_recyclable_object():
    config = {"source": 0, "device":'cpu', 'stream':True}
...
def main():
    ...
    while True:
        if (object_detector.is_cardboard and (8 > proximity_sensor.current_distance > 0) and not braccio_arm.is_picking_up):
            is_picking_up = braccio_arm.pickUp()
            
            if is_picking_up == False:
                object_detector.is_cardboard = False
